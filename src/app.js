...
try {
  ...
  logger.warn('⚠️ Fichier init-db-render.js non trouvé, initialisation manuelle requise');
...
// Middleware de logging pour diagnostiquer CORS et suivre les requêtes
app.use((req, res, next) => {
  logger.info(`🌐 ${req.method} ${req.path} - Origin: ${req.headers.origin || 'Aucune'}`);
  next();
});
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    if (allowedOrigins.includes(origin)) {
      if (process.env.NODE_ENV === 'development') {
        logger.info(`✅ CORS autorisé: ${origin}`);
      }
      callback(null, true);
    } else {
      if (process.env.NODE_ENV === 'development') {
        logger.info(`🚫 CORS bloqué pour l'origine: ${origin}`);
...
async function verifierBaseDeDonnees() {
  try {
    ...
    logger.error('❌ Erreur lors de la vérification de la base de données:', error.message);
    return false;
  }
}
...
async function initialiserBaseDeDonneesSiNecessaire() {
  try {
    ...
    if (!estInitialisee) {
      logger.info('🔄 Base de données non initialisée. Initialisation en cours...');
    ...
    logger.info('⚠️ L\'application va continuer, mais certaines fonctionnalités peuvent ne pas fonctionner.');
    logger.info('💡 Exécutez manuellement: npm run db:init-render');
  }
}
...
if (require.main === module) {
  ...
  initialiserBaseDeDonneesSiNecessaire()
    .then(() => {
      ...
      app.listen(PORT, () => {
        logger.info(`🚀 Serveur MoneyWise démarré sur le port ${PORT}`);
    ...
    .catch((error) => {
      ...
      app.listen(PORT, () => {
        logger.warn(`⚠️ Serveur démarré en mode dégradé sur le port ${PORT}`);
        logger.info(`📊 API disponible sur http://localhost:${PORT}/api`);
        logger.info('💡 Vérifiez les logs ci-dessus pour les erreurs de base de données');
      });
    });
}
